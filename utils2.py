# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 16:21:46 2024
...
class CountAtomNum:
    ...
    def count_atom_num(self, rot_layerp_sub_q, height, stick_point=None):
        ...
        if stick_point:
            if self.n == self.m:
                print('3-1',self.atomo)
                hz = rot_layerp_sub_q[0][-1]
                atomo_with_hz = np.hstack((self.atomo, np.array(hz)))
                lpq = atomo_with_hz[np.newaxis, :]
                print(lpq, lpq.shape)
                print('3-2',self.atomo)
                #lpq = self.atomo[np.newaxis,:]
                #l1a = atomo[np.newaxis,:]
                #l2a = atomo[np.newaxis,:]
            else:
                #AA stacking:
                print('4-1',self.atomo)
                hz = lpq[0][-1]
                atomo_with_hz = np.hstack((self.atomo, np.array(hz)))
                lpq = np.vstack((atomo_with_hz, lpq))
                print('4-2',self.atomo)
