# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 14:10:34 2024
...
class Lattice:
    ...
    def get_data(self):
        ...
        self.a1 = np.array([lattice_vec[0][0], 0.0])
        self.a2 = np.array([self.a1[0]*np.cos(np.pi*2/3), self.a1[0]*np.sin(np.pi*2/3)])
...
class LayerSubAtom():
    def __init__(self, layerp_sub_q, **kwargs):
        ...
        self.rot_mat1 = np.array([[np.cos(self.t1/2),-np.sin(self.t1/2)],  #for layer 1
                                  [np.sin(self.t1/2), np.cos(self.t1/2)]])
        
        self.rot_mat2 = np.array([[ np.cos(self.t1/2),np.sin(self.t1/2)],   #for layer 2
                                  [-np.sin(self.t1/2),np.cos(self.t1/2)]])
...
class TwistLayer:
    ...
    def coincident_atom(self, rot_layerp_sub_q, rot_layerr_sub_s):
        ...
        print(rot_layerp_sub_q.shape, rot_layerp_sub_q.dtype)
        print(rot_layerr_sub_s.shape, rot_layerr_sub_s.dtype)
        
        self.rot_layer = self.concatenate_sublattices()
        nrows, ncols = self.rot_layer.shape
        dtype={'names':['f{}'.format(i) for i in range(ncols)],
               'formats':ncols * [self.rot_layer.dtype]}
        ...
        self.coincident_12 = np.intersect1d(rot_layerp_sub_q.view(dtype), rot_layerr_sub_s.view(dtype)) #combine layer1 and layer2 first
        ...
        self.coincident_12 = self.coincident_12.view(self.rot_layer.dtype).reshape(-1, ncols)
